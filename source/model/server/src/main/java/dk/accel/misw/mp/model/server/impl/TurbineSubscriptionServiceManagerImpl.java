...
class TurbineSubscriptionServiceManagerImpl implements TurbineSubscriptionServiceManager, TurbineSubscriptionServiceManagerAdmin {
	...
	TurbineSubscriptionServiceManagerImpl(String name, SubscriptionMultiplexer subscriptionMultiplexer, BroadcastServer broadcastServer) {
		this.name = StdUtil.checkForNull(name);
		this.subscriptionMultiplexer = StdUtil.checkForNull(subscriptionMultiplexer);
		this.broadcastServer = StdUtil.checkForNull(broadcastServer);
	}
	...
	public void registerNode(TurbineId turbineId, NodeSubscriptionService nodeSubscriptionService) throws RemoteException {
		...
		TurbineObserverSvcImpl observer;
		if (oldService != null) {
			LOG.info("Unregistered node for " + turbineId);
			oldService.stop();
			observer = oldService.getObserver();
		} else {
			observer = new TurbineObserverSvcImpl(turbineId, subscriptionMultiplexer);
		}
		TurbineSubscriptionSvc newService = new TurbineSubscriptionSvc(turbineId, nodeSubscriptionService, observer, subscriptionMultiplexer);
	...
	public void start() {
		try {
			...
			LocateRegistry.getRegistry().bind(name, UnicastRemoteObject.exportObject(this, 0));
		} catch (Exception e) {
