...
class TurbineSubscriptionSvc implements Runnable {
	...
	TurbineSubscriptionSvc(TurbineId turbineId, NodeSubscriptionService nodeSubscriptionService,
		...
		this.nodeSubscriptionService = StdUtil.checkForNull(nodeSubscriptionService);
	...
	public void run() {
		try {
			...
			nodeSubscriptionService.updateSubscription(subscriptionInfo, observer.getRemote());
