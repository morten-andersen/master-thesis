...
class TurbineSubscriptionSvc implements Runnable {
	...
	private final TurbineObserverSvcImpl topicListener;
	...
	TurbineSubscriptionSvc(TurbineId turbineId, NodeSubscriptionService nodeSubscriptionService,
			SubscriptionMultiplexer subscriptionMultiplexer, TurbineObserverSvcImpl topicListener) {
		...
		this.topicListener = Preconditions.checkNotNull(topicListener);
	...
	TurbineObserverSvcImpl getTopicListener() {
	...
	public void run() {
		try {
			...
			nodeSubscriptionService.updateSubscription(subscriptionInfo);
